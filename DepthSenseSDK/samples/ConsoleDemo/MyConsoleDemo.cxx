...
/*    printf("frame format: %d\n", data.captureConfiguration.frameFormat); //640x480
    printf("compression: %d\n", data.captureConfiguration.compression); //640x480
    printf("value: %d\n", *(data.colorMap));
    printf("value: %d\n", *(data.colorMap+sizeof(char)));
    printf("value: %d\n", *(data.colorMap+2*sizeof(char)));
    */
   ...
   /* FILE *fp = fopen("first.ppm", "wb"); // b - binary mode 
    ...
    for (int i = 0; i < 640*480; ++i)
    {
		...
		color[2] = *(data.colorMap+3*i*sizeof(char));  // red 
		color[1] = *(data.colorMap+(3*i+1)*sizeof(char));  // green 
		color[0] = *(data.colorMap+(3*i+2)*sizeof(char));  // blue 
...
void onNewDepthSample(DepthNode node, DepthNode::NewSampleReceivedData data)
{
    ...
    /* 
    FILE *fp = fopen("depth.ppm", "wb"); // b - binary mode 
    (void) fprintf(fp, "P6\n%d %d\n255\n", 640, 480);
    for (int i = 0; i < 640*480; ++i)
    {
		static unsigned char color[3];
		color[0] = *(data.phaseMap+i*sizeof(int16_t));  // red 
		//color[1] = *(data.phaseMap+(i+1)*sizeof(char));  // green 
		//color[0] = *(data.phaseMap+(i+2)*sizeof(char));  // blue 
		(void) fwrite(color, 1, 3, fp);
    }
    (void) fclose(fp);
    */
